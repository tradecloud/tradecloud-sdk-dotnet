...
namespace Com.Tradecloud1.SDK.Client
{
    ...
    {   
        ...
        const string assigneeCompanyId = "{companyId}"; // Tasks for this buyer or supplier.
        const string orderIdToFind = "{buyerId}-{purchaseOrderNumber}"; // Find all tasks related to this `orderId` for above buyer or supplier.
        ...
        static async Task Main(string[] args)
        {
            ...
            async Task<JObject> SearchWorkflow(int offset)
            {
                ...
                var query = queryTemplate.Replace("{companyId}", assigneeCompanyId).Replace("{offset}", offset.ToString()).Replace("{limit}", limit.ToString());
